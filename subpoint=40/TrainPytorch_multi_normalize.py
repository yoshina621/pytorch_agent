...
class TrainPytorch_multi_normalize(object):
	# hyper parameters
	...
	DirName = "multiHead_normalize_multiPlay" # dir name
	...
	def select_action(self, state):
		...
		if sample > eps_threshold:
			...
			max_totalQ_index = (offenceQvalues/float(offenceQvalues.max(1)[0].view(1,1)) + diffenceQvalues/float(diffenceQvalues.max(1)[0].view(1,1))).max(1)[1].view(1,1)  #same mean
